...
"""
Eurorack quantization library, originally written for EuroPi
"""

from .hardware import MIN_OUTPUT_VOLTAGE, MAX_OUTPUT_VOLTAGE

VOLTS_PER_OCTAVE = 1.0  #: 1.0V/O is the Eurorack/Moog standard, but Buchla uses 1.2V/O
SEMITONES_PER_OCTAVE = 12  #: Standard western music scale has 12 semitones per octave
VOLTS_PER_SEMITONE = float(VOLTS_PER_OCTAVE) / float(SEMITONES_PER_OCTAVE)  #: How many volts per semitone
SEMITONE_LABELS = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]  #: Labels for the 12 semitones (using sharps, not flats)
...
class Quantizer:
    ...
    def __init__(self, notes: list[bool]=None, name: str=""):
        if notes is None:
    ...
    def __setitem__(self, n: int, value: bool) -> None:
    ...
    def quantize(self, analog_in: float, root: int=0) -> tuple[float, int]:
        """
